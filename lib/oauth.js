...
exports.v2 = function (settings) {
    ...
    return async function (request, h) {
        ...
        if (!request.query.code) {
            ...
            let nonce;
            let query = {};
            if (settings.provider.useJWSForAuth) {
                const authInfo = await settings.provider.authRequest(request);
                query.request = authInfo.token;
                nonce = authInfo.nonce;
            } else {
                nonce = Cryptiles.randomString(internals.nonceLength);
        ...
        if (settings.provider.usePrivateKey) {
            query.client_assertion_type = settings.clientAssertionType;
            query.client_assertion = await settings.clientAssertion();
        }
        ...
        if (!settings.provider.useParamsAuth && !settings.provider.usePrivateKey) {
