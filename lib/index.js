...
internals.schema = Joi.object({
    provider: Joi.object({
        ...
        usePrivateKey: internals.flexBoolean.default(false).when('protocol', { is: 'oauth2', then: Joi.optional(), otherwise: Joi.forbidden() }),
        useJWSForAuth: internals.flexBoolean.default(false).when('protocol', { is: 'oauth2', then: Joi.optional(), otherwise: Joi.forbidden() }),
        ...
        .when('useJWSForAuth', { is: true, then: Joi.forbidden() }),
        scopeSeparator: Joi.string().when('protocol', { is: 'oauth2', otherwise: Joi.forbidden() }),
        authRequest: Joi.func().when('useJWSForAuth', { is: true, otherwise: Joi.forbidden() })
    }).required(),
    password: Joi.string().required(),
    clientId: Joi.alternatives().when('provider.useJWSForAuth', {
        is: true,
    ...
    clientAssertion: Joi.func().when('provider.usePrivateKey', { is: true, otherwise: Joi.forbidden() }),
    clientAssertionType: Joi.string().when('provider.usePrivateKey', { is: true, otherwise: Joi.forbidden() }),
    cookie: Joi.string(),
    ...
    providerParams: Joi.alternatives().try(Joi.object(), Joi.func()).when('provider.useJWSForAuth', { is: true, then: Joi.forbidden() }),
